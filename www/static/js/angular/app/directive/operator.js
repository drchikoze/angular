angular.module('filters').directive('operatorLocal', ['$rootScope', 'templateLoader', 'dataSource', function($rootScope, templateLoader,$dataSource) {
    ...
    return {
        ...
        controller: function($scope, $element, $attrs, $location, $http) {
            ...
            $scope.setOperatorsOpen = function() {
                $rootScope.operatorsOpen = !$rootScope.operatorsOpen;
                $rootScope.$broadcast('changeShowOperators',  $rootScope.operatorsOpen);
                setCookie('operatorsOpen', $rootScope.operatorsOpen, {expires: 14*24*60*60});
            ...
            if (!$rootScope.operatorsOpen) {
                $rootScope.searchParams[$scope.nick] = [];
            }
            ...
            this.updateFilterStatus = function(items) {
                ...
                if ($scope.total_selected > 0) {
                    ...
                    if ($scope.total_selected == 1) {
                        ...
                        for (idx in items) {
                            if (items[idx].id == String(selected[0]))
                        ...
                        var filterOperatorsString = $scope.get_ending($scope.total_selected, ['операторов', 'оператор', 'оператора']);
                        valueString += $scope.total_selected.toString() + " " + filterOperatorsString;
                        if ($attrs['selected'])
            ...
            $scope.selectWithoutToggle = function (item) {
                $rootScope.changedItemOffsetTop = $('#item_op_' + item.id).offset().top;
                $scope.selected[item.id] = !$scope.selected[item.id];
                $scope.setValues();
            };
            ...
            $rootScope.$on('$operatorsReset', function () {
                $scope._reset();
            });
