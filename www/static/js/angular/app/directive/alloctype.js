angular.module('filters').directive('alloctype', ['$rootScope', 'templateLoader', function($rootScope, templateLoader) {
        ...
        controller: function($scope, $element, $attrs, $transclude, $location) {
            ...
            $scope.$watch('source', function (newVal) {
                ...
                if (newVal) {
                    ...
                    if ($rootScope.searchParams.apv.length < 2) {
                        for (var idx in newVal['alloc_place_value']) {
                            for (var i in newVal['alloc_place_value'][idx]) {
                                if (newVal['alloc_place_value'][idx][i]['selected']) {
                                    $scope.value = $scope.source['alloc_place_value'][idx][i]['name'];
                                }
                            }
                        }
                    } else {
                        $scope.selectedNumber = $rootScope.searchParams.apv.length;
                    }

                    if ($scope.selectedNumber > 1) {
                        $scope.value = 'Выбрано ' + $scope.selectedNumber;
                    }
            ...
            $scope.getValues = function() {
                return $rootScope.searchParams[$scope.nick];
            };
            ...
            $scope.$on('$locationChangeSuccess', function() {
                ...
                if ($scope.source !== null) {
                    for (var idx in $scope.source['alloc_place_value']) {
                        for (var i in $scope.source['alloc_place_value'][idx]) {
                            if ($scope.source['alloc_place_value'][idx][i]['id'] == $rootScope.searchParams[$scope.nickValues]) {
                                $scope.value = $scope.source['alloc_place_value'][idx][i]['name'];
                            }
                        }
                    }
            ...
            var updateLocation = function(value) {
                ...
                if (data[$scope.nick] != value) {
                    data.setValue($scope.nick, value);
                    data.setValue($scope.nickValues, []);
                    data.setUrl($rootScope.isIndexPage);
                }
            };

            var updateAllocValue = function(value) {
                var data = $rootScope.searchParams;
                data.setValue($scope.nickValues, value);
                data.setUrl($rootScope.isIndexPage);
            };
            ...
            $scope.selectAllocValue = function(nick, item) {
                var data = $rootScope.searchParams;
                ...
                for (var idx in $scope.selected) {
                    if ($scope.selected[idx]) {
                        idx_of_items.push(idx);
                    }
                }

                data[nick] = idx_of_items;
                data.setUrl($rootScope.isIndexPage);
            };
